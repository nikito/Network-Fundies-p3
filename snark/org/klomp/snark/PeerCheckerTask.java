...
class PeerCheckerTask extends TimerTask
{
    ...
    private long avgUploadBandwidth;
    ...
    public void run ()
    {
        synchronized (coordinator.peers) {
            ...
            // instantiate avg bandwidth
            avgUploadBandwidth = 0;

            Iterator it = coordinator.peers.iterator();
            // Iterate through and generate the total uploaded and downloaded.
            while (it.hasNext()) {
            ...
            // Calculate the new weighted average if uploaded is greater than 0.
            if(uploaded > 0) {
            	// avg = (0.875 * newUp) + ((1 - .875) * (old avg))				
            	avgUploadBandwidth = (long) ((0.875 * uploaded) + ((1 - 0.875) * avgUploadBandwidth));
            }
            // Iterate through again and generate the peer lists.
            ...
            while (it.hasNext()) {
                ...
                long download = peer.getDownloaded();                                
                long allocation;
                if(avgUploadBandwidth == 0) {
                	allocation = Long.MAX_VALUE;
                } else {
        ...
        // do not allow there to ever be more uploaders than downloaders!
        coordinator.MAX_UPLOADERS = coordinator.peers.size() - coordinator.uploaders;
    }
